...
echo "#!/bin/sh" > /etc/profile.d/andronix.sh
echo "unset LD_PRELOAD" >> /etc/profile.d/andronix.sh
echo "unset LD_LIBRARY_PATH" >> /etc/profile.d/andronix.sh
echo "export LIBGL_ALWAYS_SOFTWARE=1" >> /etc/profile.d/andronix.sh
chmod +x /etc/profile.d/andronix.sh

#update our repos so we can install some packages
apt-get update

#install some packages with need for Andronix
